...
public abstract class AbstractEtlService {
    ...
    private final long    CNT_PER_TASK = 3000L;
    ...
    protected EtlResult importData(String sql, List<String> params) {
        ...
        try {
            ...
            if (config.getMapping().getEtlCondition() != null && params != null) {
                // etlCondition可以添加多个条件，格式为"where id > {} and code {}",对应的params参数格式"2; achao"
            ...
            if (cnt >= 10000) {
                ...
                // 初始化线程池
                ExecutorService executor = Util.newFixedThreadPool(threadCount, 5L);
                List<Future<Boolean>> futures = new ArrayList<>();
                // 重新设置线程池的大小
                dataSource.setMaxActive(threadCount);
                // 往线程池里面submit workerCnt个线程
                for (long i = 0; i < workerCnt; i++) {
                    offset = size * i;
                    String sqlFinal = sql + " LIMIT " + offset + "," + size;
                    if (logger.isDebugEnabled()) {
                        logger.debug("sqlFinal is: " + sqlFinal);
                    }
